...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        sig_mul = t1.f.mul_zip(sig, grad_output)
        sig_mul2 = t1.f.mul_zip(sig, sig)
        return t1.f.add_zip(
            sig_mul,
            t1.f.neg_map(t1.f.mul_zip(sig_mul2, grad_output)),
        )
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        ...
        ctx.save_for_backward(a, order_lst)
        new_tensor_data = a._tensor.permute(*order_lst)
        return minitorch.Tensor.make(
            new_tensor_data._storage,
            new_tensor_data.shape,
            new_tensor_data.strides,
            a.backend
        )
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        order_lst = np.argsort(order)
        new_grad_data = grad_output._tensor.permute(*order_lst)
        return (
            minitorch.Tensor.make(
                new_grad_data._storage,
                new_grad_data.shape,
                new_grad_data.strides,
                grad_output.backend,
            ), 0.0
        )
